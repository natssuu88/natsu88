...
rclone copy out/target/product/$(grep unch $CIRRUS_WORKING_DIR/build.sh -m 1 | cut -d ' ' -f 2 | cut -d _ -f 2 | cut -d - -f 1)/*.zip natsu:$(grep init $CIRRUS_WORKING_DIR/build.sh -m 1 | cut -d / -f 4)/$(grep unch $CIRRUS_WORKING_DIR/build.sh -m 1 | cut -d ' ' -f 2 | cut -d _ -f 2 | cut -d - -f 1) -P
...
time com ccache 1
rclone copy --drive-chunk-size 256M --stats 1s ccache.tar.gz natsu:ccache/$name_rom/$device -P
rm -rf ccache.tar.gz
echo ━━━━━━━━━ஜ۩۞۩ஜ━━━━━━━━
